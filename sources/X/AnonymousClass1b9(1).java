package X;

import android.os.Bundle;
import android.view.LayoutInflater;
import android.view.View;
import android.view.ViewGroup;
import com.instagram.barcelona.R;
import com.instagram.service.session.UserSession;
import java.util.HashMap;

/* renamed from: X.1b9  reason: invalid class name */
public final class AnonymousClass1b9 extends C34640IcN implements C82424pb {
    public static final String __redex_internal_original_name = "CheckoutUpsellValuePropFragment";
    public boolean A00 = false;
    public UserSession A01;

    public final String getModuleName() {
        return "CHECKOUT_VALUE_PROP_FRAGMENT";
    }

    public final boolean onBackPressed() {
        return false;
    }

    public final C10300i6 getSession() {
        return this.A01;
    }

    public final void onCreate(Bundle bundle) {
        int A02 = C14030oh.A02(-148226158);
        super.onCreate(bundle);
        UserSession A0V = C18180wK.A0V(C18250wR.A07(this));
        this.A01 = A0V;
        C130667qT A0T = C18220wO.A0T(requireActivity(), new C683443k(this), A0V);
        UserSession userSession = this.A01;
        Bundle A07 = C18250wR.A07(this);
        String string = A07.getString("waterfall_id");
        String string2 = A07.getString("entry_point");
        String string3 = A07.getString("prior_module");
        K9R k9r = K9R.A00;
        IVw iVw = new IVw(k9r);
        if (string2 == null) {
            string2 = "";
        }
        iVw.A04("entry_point", string2);
        if (string == null) {
            string = "";
        }
        iVw.A04("waterfall_id", string);
        if (string3 == null) {
            string3 = "";
        }
        iVw.A04("prior_module", string3);
        IVw A0M = C18180wK.A0M(k9r, iVw);
        HashMap A0y = AnonymousClass0wJ.A0y();
        C18200wM.A1T(A0M, A0y);
        AnonymousClass4A9 A002 = C63263h8.A00(userSession, "com.bloks.www.bloks.commerce.onboarding.open.checkout.value.prop.async", A0y);
        AnonymousClass4A9.A01(A002, A0T, this, 21);
        schedule(A002);
        C14030oh.A09(-804566667, A02);
    }

    public final View onCreateView(LayoutInflater layoutInflater, ViewGroup viewGroup, Bundle bundle) {
        int A02 = C14030oh.A02(-594326132);
        View A0H = AnonymousClass0wJ.A0H(layoutInflater, viewGroup, R.layout.layout_recyclerview);
        C14030oh.A09(1147924109, A02);
        return A0H;
    }

    public final void onResume() {
        int A02 = C14030oh.A02(643714986);
        super.onResume();
        if (this.A00) {
            C18190wL.A19(this);
        }
        C14030oh.A09(-1071377151, A02);
    }
}
